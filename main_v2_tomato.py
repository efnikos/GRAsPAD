...
detector = tomato_detector.TomatoDetector(model_path='models/keypoints_new.pt', show_frame=False)
#detector.run()
...
z = np.zeros(9)
measurement_uncertainty  = np.eye(9)
    ...
    try:
        ...
        if keypoints:
            for idx, kp in keypoints.items():
                ...
                z[idx+3] = p_te[1]         # This gives you the 3D coordinates, e.g., [x, y, z]
                z[idx+6] = p_te[2]         # This gives you the 3D coordinates, e.g., [x, y, z]
                
                # Find the diagonal position to update
                diag_index = idx * 3  # For idx=0, it's the first 3 positions; for idx=1, the next 3 positions, etc.
                # Fill the corresponding diagonal block with the keypoint's confidence
                np.fill_diagonal(measurement_uncertainty[diag_index:diag_index+3, diag_index:diag_index+3], 1-kp["confidence"])
        ...
        iterat = 1
        if iterat == 1:
            x = np.array([-0.02, -0.02, -0.01, -0.61, -0.68,-0.67, 0.38,0.4,0.47]).T
        ...
        projected_point_tag0 = np.linalg.inv(T_des) @ np.hstack((x_hat[[0, 3, 6]],[1]))
        projected_point_tag1 = np.linalg.inv(T_des) @ np.hstack((x_hat[[1, 4, 7]],[1]))
        projected_point_tag2 = np.linalg.inv(T_des) @ np.hstack((x_hat[[2, 5, 8]],[1]))

        # Project the 3D point onto the 2D pixel plane
        pixelCoordstag0 = K @ projected_point_tag0[0:3]
        pixelCoordstag1 = K @ projected_point_tag1[0:3]
        pixelCoordstag2 = K @ projected_point_tag2[0:3]
        ...
        bl_diag = block_diag(T_des[0:3, 0:3], T_des[0:3, 0:3], T_des[0:3, 0:3])
        ...
        P = (np.eye(9) - KALMAN_GAIN @ H) @ P_pred    # Updated estimate covariance
